...
;(function(window, undefined) {
  ...
  function concat() {
    ...
    while (++j < length) {
      ...
      if (isClassOf(value, 'Array')) {
        for (var k = 0, l = value.length; k < l; k++, i++) {
          if (k in value) {
            result[i] = value[k];
          }
        }
  ...
  function reverse() {
    ...
    if (length > 1) {
      while (++i < middle) {
        upperIndex = length - i - 1;
        value = upperIndex in object ? object[upperIndex] : uid;
        if (i in object) {
          object[upperIndex] = object[i];
        } else {
  ...
  function forProps() {
    ...
    forProps = function(object, callback, options) {
      ...
      if (allFlag && support.getAllKeys) {
        for (i = 0, keys = getAllKeys(object), length = keys.length; i < length; i++) {
          key = keys[i];
      ...
      // else iterate only enumerable properties
        ...
        if (!done && (forArgs && isArguments(object) ||
          ...
          while (++i < length) {
            if ((done =
                callback(iteratee[i], String(i), object) === false)) {
              break;
            }
          }
        }
  ...
  function deepClone(value) {
    ...
    // remove markers
    for (i = 0, length = marked.length; i < length; i++) {
      data = marked[i];
    ...
    if (length === length >>> 0) {
      if (isConvertable) {
        ...
        callback = function(value, index) {
          return fn.call(this, value, i, origObject);
        };
        // in IE < 9 strings don't support accessing characters by index
        if (isSplittable) {
          object = object.split('');
          ...
          while (++i < length) {
            ...
            object[i] = isSnapshot ? result.snapshotItem(i) : result[i];
          }
        }
      }
  ...
  function filter(array, callback, thisArg) {
    ...
    return result || reduce(array, function(result, value, index) {
      return callback.call(thisArg, value, i, array) ? (result.push(value), result) : result;
    }, []);
  }
  ...
  function forEach(array, callback, thisArg) {
    ...
    while (++i < length) {
      if (i in array &&
          callback(array[i], i, array) === false) {
        break;
      }
    }
    return array;
  ...
  function indexOf(array, value, fromIndex) {
    ...
    i = (i < 0 ? max(0, length + i) : i) - 1;
    while (++i < length) {
      if (i in array && value === array[i]) {
        return i;
      }
    }
    return -1;
  ...
  function invoke(benches, name) {
    ...
    function execute() {
      ...
      // execute method
      result[i] = isClassOf(bench && bench[name], 'Function') ? bench[name].apply(bench, args) : undefined;
    ...
    function getNext(event) {
      ...
      if (!cycleEvent.aborted && raiseIndex() !== false) {
        bench = queued ? benches[0] : result[i];
        if (isAsync(bench)) {
          delay(bench, execute);
        }
        else if (async) {
    ...
    function raiseIndex() {
      ...
      if (queued) {
        ...
        do {
          ++i > 0 && shift.call(benches);
        } while ((length = benches.length) && !('0' in benches));
      }
      else {
        while (++i < length && !(i in result)) { }
      }
      // if we reached the last index then return `false`
      return (queued ? length : i < length) ? i : (i = false);
    }
  ...
  function map(array, callback, thisArg) {
    return reduce(array, function(result, value, index) {
      result[i] = callback.call(thisArg, value, i, array);
      return result;
  ...
  function reduce(array, callback, accumulator) {
    ...
    forEach(array, function(value, index) {
      accumulator = noaccum ? (noaccum = false, value) : callback(accumulator, value, i, array);
    });
    return accumulator;
  }
